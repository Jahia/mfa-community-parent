...
function querySuccess(settings) {
    return {
        ...
        result: {data: {mfaTotp: {siteSettings: settings}}}
    };
}
...
describe('SiteSettings - load failure', () => {
    it('renders a blocking error state and does not render the editable form', async () => {
        ...
        expect(await screen.findByTestId('site-settings-load-error')).toBeInTheDocument();
        ...
        expect(screen.queryByTestId('site-enabled-toggle')).not.toBeInTheDocument();
        expect(screen.queryByTestId('site-groups-input')).not.toBeInTheDocument();
    });

    it('keeps Save disabled so the wipe-on-error cannot be triggered', async () => {
        renderSiteSettings([queryFailure()]);

        await screen.findByTestId('site-settings-load-error');

        expect(screen.getByTestId('site-settings-save-btn')).toBeDisabled();
    });
});
...
describe('SiteSettings - Save gating on load', () => {
    it('disables Save while the query is loading, then enables it once the settings have populated the form', async () => {
        ...
        await screen.findByTestId('site-enabled-toggle');

        await waitFor(() => {
            expect(screen.getByTestId('site-settings-save-btn')).toBeEnabled();
        });
    });
});
...
describe('SiteSettings - successful load', () => {
    it('populates the form fields from the server response', async () => {
        ...
        expect(await screen.findByTestId('site-enabled-toggle')).toBeChecked();
        expect(screen.getByTestId('site-groups-input')).toHaveValue('editors, reviewers');
    });
    ...
    it('leaves the checkbox unchecked and the groups field empty when the site has no settings yet', async () => {
        ...
        expect(await screen.findByTestId('site-enabled-toggle')).not.toBeChecked();
        expect(screen.getByTestId('site-groups-input')).toHaveValue('');
    });
});

describe('SiteSettings - save path', () => {
    it('submits the currently displayed values as mutation variables, without loginUrl/logoutUrl', async () => {
        ...
        await screen.findByTestId('site-enabled-toggle');
        await user.click(screen.getByTestId('site-settings-save-btn'));

        expect(await screen.findByTestId('site-settings-saved')).toBeInTheDocument();
    });
    ...
    it('filters blank entries and trims whitespace out of the comma-separated groups field', async () => {
        ...
        await screen.findByTestId('site-enabled-toggle');
        await user.clear(screen.getByTestId('site-groups-input'));
        await user.type(screen.getByTestId('site-groups-input'), ' editors ,, reviewers ,  ');
        await user.click(screen.getByTestId('site-settings-save-btn'));
    ...
    it('sends an empty enabledGroups list when the site is disabled, even if the field has stale text', async () => {
        ...
        await screen.findByTestId('site-enabled-toggle');
        await user.click(screen.getByTestId('site-enabled-toggle'));
        await user.click(screen.getByTestId('site-settings-save-btn'));
    ...
    it('surfaces a mutation error to the admin instead of swallowing it', async () => {
        ...
        expect(await screen.findByTestId('site-settings-error')).toBeInTheDocument();
        expect(screen.queryByTestId('site-settings-saved')).not.toBeInTheDocument();
        await waitFor(() => {
            expect(screen.getByTestId('site-settings-save-btn')).toBeEnabled();
        });
    });
