...
describe('AuditReportSection - empty results', () => {
    it('shows the "no events" message when the audit query returns no rows', async () => {
        ...
        const mock = {
            ...
            result: {data: {mfaTotp: {auditEvents: []}}}
        };
        renderSection([mock]);
    ...
    it('shows the "no enrollment data" message when the report query returns nothing', async () => {
        const user = userEvent.setup();
        const mock = {
            request: {query: EnrollmentReportQuery, variables: {siteKey: SITE_KEY, limit: 200}},
            result: {data: {mfaTotp: {enrollmentReport: null}}}
        };
        renderSection([mock]);
        ...
        const mock = {
            ...
            result: {
                data: {
                    mfaTotp: {
                        auditEvents: [
                            {eventType: 'ENROLL', outcome: 'SUCCESS', userId: 'alice', siteKey: SITE_KEY, timestamp: '1700000000000', detail: 'first enrollment'},
                            {eventType: 'LOGIN', outcome: 'FAILURE', userId: 'bob', siteKey: SITE_KEY, timestamp: '1700000100000', detail: 'invalid code'}
                        ]
                    }
                }
            }
...
describe('AuditReportSection - error paths', () => {
    ...
    it('surfaces an enrollment report query failure to the admin instead of showing "no report"', async () => {
        const user = userEvent.setup();
        const mock = {
            request: {query: EnrollmentReportQuery, variables: {siteKey: SITE_KEY, limit: 200}},
            error: new Error('network down')
